document.addEventListener('DOMContentLoaded', function() {
  ...
  // Existing: Strict grid reveal on scroll (keep as is)
  (function () {
    ...
    const elementsToReveal = document.querySelectorAll(
        '.section-title, .feature-card, .video-container, .syllabus-item, .map-container, .profile-card'
    );
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key >= '1' && e.key <= '4' && !e.ctrlKey && !e.altKey && !e.shiftKey) {
            if (e.target.tagName !== 'INPUT' && e.target.tagName !== 'TEXTAREA') {
                const sections = ['#hero', '#features', '#syllabus', '#contact'];
...
const schemaData = {
    ...
    "@graph": [
        ...
        },
        {
            ...
            "jobTitle": "Stock Market Educator",
            "worksFor": {
                "@type": "Organization",
                "name": "Shrvan Wealth Creation"
            },
...
document.addEventListener('click', (e) => {
    if (e.target.matches('.btn, .social-link, .floating-cta')) {
